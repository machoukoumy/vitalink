          ...
          {notifications.map(n => (
            <div key={n.id} onClick={() => !n.isRead && markRead(n.id)}
              className={cn(
                ...
                n.isRead ? "border-gray-100" : "border-blue-200 bg-blue-50/50"
              )}>
              <div className="flex items-start justify-between">
                <div>
                  <h4 className={cn("font-medium", n.isRead ? "text-gray-700" : "text-gray-900")}>{n.title}</h4>
                  <p className="text-sm text-gray-600 mt-1">{n.message}</p>
                </div>
                <span className="text-xs text-gray-400 whitespace-nowrap ml-4">{formatDateTime(n.createdAt)}</span>
              </div>
            </div>
          ))}
        </div>
