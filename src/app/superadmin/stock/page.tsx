...
export default function SuperAdminStockPage() {
  ...
  const columns = [
    ...
    { key: "center", label: "Centre", render: (s: StockRecord) => s.center?.name || "-" },
    ...
    { key: "storageLocation", label: "Emplacement", render: (s: StockRecord) => s.storageLocation || "-" },
    ...
    { key: "actions", label: "Actions", render: (s: StockRecord) => (
      <select value={s.status} onChange={e => handleStatusChange(s.id, e.target.value)}
        ...
        {STOCK_STATUSES.map(st => <option key={st} value={st}>{st}</option>)}
      </select>
    )},
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Stock Sanguin Global</h1>
        <p className="text-gray-500 mt-1">Vue d&apos;ensemble du stock de tous les centres</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <DataTable columns={columns} data={stocks} emptyMessage="Aucun stock enregistr&eacute;" />
          {totalPages > 1 && (
